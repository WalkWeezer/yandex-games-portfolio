...
window.FEEL_DEMOS["deadline-escape"] = {
  ...
  ensureArt() {
    ...
    ["floor_a", "floor_b", "desk", "desk2", "plant", "cooler", "fog", "wall", "window", "cabinet", "printer", "trash"].forEach((t) => tryLoad("tile_" + t, `frames/tile_${t}.png?v=vfxcut1`));
    // каркас: прямые + L + stub (+ U в запасе)
    ["n", "s", "e", "w"].forEach((d) => {
      tryLoad("tile_wall_" + d, `frames/tile_wall_${d}.png?v=vfxcut1`);
      tryLoad("tile_window_" + d, `frames/tile_window_${d}.png?v=vfxcut1`);
    });
    ["nw", "ne", "sw", "se"].forEach((d) => {
      tryLoad("tile_wall_" + d, `frames/tile_wall_${d}.png?v=vfxcut1`);
      tryLoad("tile_window_" + d, `frames/tile_window_${d}.png?v=vfxcut1`);
      tryLoad("tile_wall_stub_" + d, `frames/tile_wall_stub_${d}.png?v=vfxcut1`);
      tryLoad("tile_window_stub_" + d, `frames/tile_window_stub_${d}.png?v=vfxcut1`);
    });
    ...
    ["shield", "steam", "invuln", "near_miss", "report", "dash", "slam", "confetti"].forEach((v) => tryLoad("vfx_" + v, `frames/vfx_${v}.png?v=vfxcut1`));
    this._art = art;
    return art;
  },
  dirKey(dir) {
