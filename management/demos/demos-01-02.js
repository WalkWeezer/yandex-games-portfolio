...
window.FEEL_DEMOS["deadline-escape"] = {
  ...
  ensureArt() {
    ...
    ["floor_a", "floor_b", "desk", "desk2", "plant", "cooler", "fog", "cabinet", "printer", "trash"].forEach((t) => tryLoad("tile_" + t, `frames/tile_${t}.png?v=w250721e`));
    // стены — proof-геометрия без спрайтов (wall/window tiles не грузим)
    ["coin", "coffee", "badge"].forEach((p) => tryLoad("pu_" + p, `frames/pu_${p}.png`));
    ["shield", "steam", "invuln", "near_miss", "report", "dash", "slam", "confetti"].forEach((v) => tryLoad("vfx_" + v, `frames/vfx_${v}.png?v=w250721e`));
    this._art = art;
    return art;
  },
  dirKey(dir) {
