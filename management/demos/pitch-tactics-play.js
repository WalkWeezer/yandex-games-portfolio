(function () {
  ...
  function chanceShot(p, goalHex, gkDiveCol) {
    ...
    let skill = skillPct(p.shot) * 0.92 * Math.max(0, 1 - 0.5 * pr.pts) * shotProximity(p.side, p.pos);
    let diveNote = "";
  ...
  function renderLineup() {
    ...
    app.innerHTML =
      ...
      '<div class="hint-box">Ход как в XCOM: <b>жёлтый</b>/<b>золотой</b> радиус. Игроки стоят где оставили — без автосдвига формы. С мячом −дебафф владения.</div></section>';
  ...
  function doShot(p, goalHex) {
    ...
    const resolveMissOrSave = () => {
      state.ball = [
        ...
        if (dive === goalHex[0] && Math.random() < 0.42) {
  ...
  function endPlayerTurn() {
    ...
    state.reachable = [];
    state.targets = [];
    // без авто-подтягивания формы — только явные команды
    state.turn = "B";
  ...
  function aiShot(ow) {
    ...
    if (scored) {
      ...
      toast("Гол ПК");
      resetKickoff("A");
    } else if (dive === aim && Math.random() < 0.42) {
  ...
  function aiSupportRun(ballOw) {
    ...
    runners.sort((a, b) => a.pos[1] - b.pos[1]);
    const r = runners[0];
    if (!r) return false;
    const targetRow = Math.max(2, Math.min(ballOw.pos[1] - 2, r.pos[1] - 2));
    const col =
      r.role === "OP1" ? clamp(r.home[0], 0, 4) : r.role === "OP2" ? clamp(r.home[0], 8, 12) : clamp(ballOw.pos[0], 3, 9);
  ...
  function aiSmart(ow) {
    ...
    if (Math.random() < 0.35 && aiSupportRun(ow)) return;
  ...
  function aiWidth(ow) {
    ...
    if (Math.random() < 0.3 && aiSupportRun(ow)) return;
  ...
  function aiPossess(ow) {
    ...
    if (Math.random() < 0.32 && press < 2 && aiSupportRun(ow)) return;
  ...
  function aiDirect(ow) {
    ...
    if (Math.random() < 0.28 && aiSupportRun(ow)) return;
  ...
  function aiPressAttack(ow) {
    ...
    if (ow.pos[1] > HALF_ROW + 1 && aiEscapeOwnHalf(ow)) return;
    if (Math.random() < 0.25 && aiSupportRun(ow)) return;
    const recv = freest(ow, true, { needForward: true });
    if (recv) {
      aiPass(ow, recv, hexDist(ow.pos, recv.pos) > actionRange(ow, "pass"));
      return;
  ...
  function playerSupportRun(ballOw) {
    ...
    runners.sort((a, b) => b.pos[1] - a.pos[1]);
    const r = runners.find((p) => p.pos[1] <= ballOw.pos[1] + 1) || runners[0];
    if (!r) return false;
    const targetRow = Math.min(GOAL_B_ROW - 2, Math.max(ballOw.pos[1] + 2, r.pos[1] + 2));
    const col =
      r.role === "OP1" ? clamp(r.home[0], 0, 4) : r.role === "OP2" ? clamp(r.home[0], 8, 12) : clamp(ballOw.pos[0], 3, 9);
  ...
  function playerAutoAction() {
    ...
    if (ow && ow.side === "A" && !isLocked(ow.id)) {
      ...
      if (Math.random() < 0.38 && press < 2 && playerSupportRun(ow)) return true;

      const shotRow = style === "possess" ? GOAL_B_ROW - 7 : GOAL_B_ROW - 9;
      const minChance = style === "possess" ? 14 : 10;
      if (ow.pos[1] >= shotRow || ow.pos[1] >= GOAL_B_ROW - 5) {
        const aim = GOAL_COLS.includes(ow.pos[0]) ? ow.pos[0] : CENTER_COL;
        const goalHex = [aim, GOAL_B_ROW];
        const ch = chanceShot(ow, goalHex, null);
        const urgent = ow.pos[1] >= GOAL_B_ROW - 5;
        if (!ch.outOfRange && ch.chance >= (urgent ? Math.min(8, minChance) : minChance) && ch.pressure < 3) {
          doShot(ow, goalHex);
          return true;
        }
      }
      ...
      const fwd = playerFreest(ow, { needForward: true });
      const wantPass =
        (fwd && (style === "possess" || press >= 1 || fwd.pos[1] > ow.pos[1] + 1)) ||
  ...
  function evaluateFootballQuality(result) {
    ...
    const dominantShots = homeStronger ? sf : awayStronger ? sa : Math.min(sf, sa);
    const weakShots = homeStronger ? sa : awayStronger ? sf : Math.min(sf, sa);
    const equalish = !mismatch && Math.abs(homeMult - awayMult) < 0.15;

    // Голы: 0:0 допустим при реальных моментах; на mismatch — осада без гола тоже бывает редко, но xG должен быть
    if (totalGoals < 1) {
      if (shots >= 5 && xgSum >= 1.0) notes.push("0:0 при моментах (xG " + xgSum.toFixed(2) + ") — низкая реализация");
      else if (bigMismatch && dominantShots >= 4 && xgSum >= 1.2)
        notes.push("безголевая осада при явном превосходстве силы");
      else issues.push("мало голов (<1)");
    }
    const spreadNoise = (result.skillSpread || 0) >= 0.85;
    if (totalGoals > 12) issues.push("много голов (>12)");
    else if (totalGoals > 9 && equalish && !spreadNoise) issues.push("много голов на равных (>9)");
    else if (totalGoals > 8) notes.push("результативный матч (открытый темп / разница классов)");

    // Удары
    if (shots < 5) {
      if (bigMismatch && dominantShots >= 3) notes.push("мало ударов суммарно — слабый почти не доходит, сила gap=" + gap.toFixed(2));
      else if (mismatch && shots >= 3 && dominantShots >= 2) notes.push("мало ударов при перевесе силы");
    ...
    if (shots >= 10 && totalGoals / shots > 0.75) issues.push("слишком высокая реализация");
    else if (shots >= 6 && totalGoals / shots > 0.78 && equalish) issues.push("слишком высокая реализация");

    if (mid > 0.82) issues.push("залипание в центре (>82%)");
    else if (mid > 0.78 && equalish && shots < 6) issues.push("залипание в центре (>78%)");
    if (att < 0.12) issues.push("мало игры в финальных третях");
    else if (att < 0.16 && equalish) issues.push("мало игры в финальных третях");

    const gd = Math.abs(s[0] - s[1]);
    const xgRatio = Math.max(xgA, xgB) / Math.max(0.35, Math.min(xgA, xgB));
    if (equalish && gd > 4) {
      if (spreadNoise && xgRatio >= 2.2)
        notes.push("разгром при skill-spread: xG " + xgA.toFixed(2) + "-" + xgB.toFixed(2));
      else if (xgRatio >= 3.0)
        notes.push("разгром по моментам (xG ratio " + xgRatio.toFixed(1) + "), не по «бумажной» силе");
      else issues.push("разгром на равных");
    }
    if (mismatch && gd >= 4) {
      const favWon = (homeStronger && s[0] > s[1]) || (awayStronger && s[1] > s[0]);
      if (favWon) notes.push("разгром оправдан разницей силы (gap=" + gap.toFixed(2) + ")");
    }

    if ((st.passAtt || 0) >= 30 && passPct < 32) issues.push("пас слишком хаотичный");
    if ((st.passAtt || 0) >= 30 && passPct > 92) issues.push("пас слишком лёгкий");

    const pressy = result.awayAi === "press" || result.homeStyle === "press";
    const tackleCap = pressy || mismatch ? 105 : 88;
    if ((st.tackles || 0) > tackleCap) issues.push("спам отборов");

    // Односторонние удары — только если слабый «задушен» превосходством
    if (sf < 1 || sa < 1) {
      if ((homeStronger || awayStronger) && mismatch && weakShots < 1 && dominantShots >= 3) {
        notes.push(
          "слабый без ударов оправдан превосходством (gap=" +
            gap.toFixed(2) +
            ", удары фаворита " +
            dominantShots +
            ")"
        );
      } else if ((homeStronger || awayStronger) && bigMismatch && weakShots < 1 && dominantShots >= 2) {
        notes.push("полный контроль фаворита — ударов у аутсайдера нет");
      } else {
        issues.push("одна сторона почти не бьёт");
      }
    }

    // Фаворит обязан создавать хоть что-то
    if (mismatch && dominantShots < 2 && totalGoals < 2) issues.push("фаворит почти не создаёт моменты");

    return { ok: issues.length === 0, issues, notes, strength: { homeMult, awayMult, gap: +gap.toFixed(3), mismatch, bigMismatch } };
  }
