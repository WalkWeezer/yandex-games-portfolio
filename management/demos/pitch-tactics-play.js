(function () {
  ...
  /**
   * S/A 1–5 → гексы за одно действие (не «перебежать поле»).
   * 1→1, 2→1, 3→2, 4→2, 5→3. С мячом ещё −1 (мин. 1).
   */
  function attrToHexes(attr) {
    const a = clamp(attr | 0, 1, 5);
    return [0, 1, 1, 2, 2, 3][a];
  }

  function moveBudget(p) {
    const raw = state.ballOwner === p.id && p.burst ? p.accel : p.speed;
    let hexes = attrToHexes(raw);
    if (state.ballOwner === p.id) hexes = Math.max(1, hexes - 1); // ведение короче спринта без мяча
    return hexes;
  }
  ...
  function canStandOn(pos, movingId) {
    ...
    if (isGoalHex(pos[0], pos[1])) return false; // в ворота только ударом
  ...
  function renderLineup() {
    ...
    app.innerHTML =
      ...
      '<div class="hint-box">S/A 1–5 → ход на 1/1/2/2/3 гекса за действие; с мячом ещё −1. За 2 AP быстрый нап проходит ~4 гекса с мячом — не полполя. Левин A5 (рывок 2 с мячом), Райцев S5 (без мяча 3).</div></section>';
  ...
  function chooseRadial(mode) {
    ...
    if (state.mode === "move") {
      ...
      const tag =
        ...
        bud +
        " гекс" +
        (state.ballOwner === p.id ? " (с мячом −1)" : "");
  ...
  function updatePreview(hoverHex) {
    ...
    if (state.mode === "move") {
      ...
      let html =
        ...
        " → " +
        attrToHexes(raw) +
        (state.ballOwner === p.id ? " −1 с мячом" : "") +
        " · S" +
        p.speed +
        "/A" +
        p.accel;
      if (state.ballOwner === p.id && hoverHex) {
        ...
        html +=
          "<div class='muted'>S/A 1–5 дают 1/1/2/2/3 гекса за действие; с мячом на гекс меньше. 2 AP ≠ пробег всего поля.</div>";
      }
      box.innerHTML = html;
      return;
    }
