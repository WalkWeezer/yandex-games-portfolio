(function () {
  ...
  function playerAutoAction() {
    ...
    if (ow && ow.side === "A" && !isLocked(ow.id)) {
      if (ow.pos[1] >= GOAL_B_ROW - 5) {
        const aim = GOAL_COLS.includes(ow.pos[0]) ? ow.pos[0] : CENTER_COL;
        const goalHex = [aim, GOAL_B_ROW];
        const ch = chanceShot(ow, goalHex, null);
        if (!ch.outOfRange && ch.chance >= 20 && ch.pressure < 2) {
          doShot(ow, goalHex);
          return true;
        }
      }
      const recv = playerFreest(ow);
      const press = pressureOn(ow.pos, "A").pts;
      if (recv && (press >= 1 || recv.pos[1] > ow.pos[1])) {
        const isCross = hexDist(ow.pos, recv.pos) > actionRange(ow, "pass");
        const mode = isCross ? "cross" : "pass";
        if (inActionRange(ow, recv.pos, mode)) {
          doPass(ow, recv, isCross);
          if (state.stats) state.stats.passes++;
          return true;
        }
      }
  ...
  /** ИИ vs ИИ, полный матч 90'. window.pitchAutoPlayFullMatch('academy') */
  function autoPlayFullMatch(oppId) {
    state.autoPlay = true;
    state.matchArchive = [];
    state.stats = { shotsFor: 0, goalsFor: 0, goalsAgainst: 0, gkSaves: 0, passes: 0, tackles: 0 };
    state.opponentId = oppId || "academy";
    const opp = OPPONENTS.find((x) => x.id === state.opponentId);
    state.you = buildSquad("A", null, 1);
    state.them = buildSquad("B", opp.names, opp.mult);
    startMatch();
