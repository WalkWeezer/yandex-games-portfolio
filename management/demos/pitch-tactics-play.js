(function () {
  ...
  function chanceTackle(p, victim) {
    ...
    let skill = skillPct(p.tackle) * (0.42 + 0.11 * pr.pts);
  ...
  function aiDefend(style) {
    ...
    if (ow && ow.side === "A" && hexDist(h.pos, ow.pos) <= range) {
      ...
      if (pr < 2 && Math.random() < (style === "press" ? 0.35 : 0.72)) {
        const shadow = [
  ...
  function evaluateFootballQuality(result) {
    ...
    const shots = (st.shotsFor || 0) + (st.shotsAgainst || 0);
    ...
    if (totalGoals < 1) issues.push("мало голов (<1)");
    if (totalGoals > 7) issues.push("много голов (>7)");
    if (shots < 5) issues.push("мало ударов (<5)");
    ...
    if ((st.shotsFor || 0) < 1 || (st.shotsAgainst || 0) < 1) issues.push("одна сторона почти не бьёт");
