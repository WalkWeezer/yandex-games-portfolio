...
> Обновлено 2026-07-18. Источник правды: `management/demos/demos-03-06.js` → `FEEL_DEMOS["legends-of-the-pitch"]`.  
> **Ключ механики:** на поле комбинируются **позиция × амплуа × характер** → наглядный бонус (линии/чипы/всплески в матче).

| Поле | Значение |
|------|----------|
| **Core verb** | Собери комбо → расставь → **смотри матч** (бег по всему полю, составы пересекаются) |
| **Пространство** | Полное поле; дома слотов в магазине; в бою свободный бег + marking, половины пересекаются |
| **Win** | Больше голов после 9 раундов |
| **Fail** | Меньше голов к финалу |
| **Ввод** | Магазин/merge/расстановка; бой зрелищный; 1× Таймаут |
| **Анти-вектор** | Статы без читаемых комбо; физика 11v11; FM-Excel |

### Три оси карты (ядро)

| Ось | Что это | Пример |
|-----|---------|--------|
| **Позиция** | зона слота на поле | GK / DEF / MID / FWD |
| **Амплуа** | специализация внутри роли | снайпер, дирижёр, стоппер, сейвер… |
| **Характер** | тег синергии | прессинг / контроль / контра / стена |

**Комбо = сочетания осей**, не просто «2 одинаковых цвета»:
- **FIT** — амплуа-роль совпала с позицией слота  
- **TRAIT 2/3** — N героев одного характера  
- **ENGINE** — MID(контроль) + FWD(контра)  
- **WALLCHAIN** — DEF(стена) + GK(стена)  
- **PRESSLINE** — DEF/MID с прессингом  

Бонус должен быть **виден на поле** (линия между героями, чип комбо, вспышка в раунде).

| Число | Значение |
|-------|----------|
| Старт | **3v3**; 🪙10; merge 3→★ |
| Оси на карте | позиция-роль · амплуа · характер · ★ |
| Комбо | FIT / TRAIT / ENGINE / WALLCHAIN / PRESSLINE |
