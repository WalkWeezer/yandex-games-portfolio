...
> Обновлено 2026-07-19. Источник правды: `management/demos/legends-pitch-demo.js` → `FEEL_DEMOS["legends-of-the-pitch"]`.  
> **Ключ механики:** карта = **1 амплуа + 1 тактика**; тактики стакаются как трейты TFT (×2/×3); матч top-down с отдельными FX на отбор/финт/удар.

| Поле | Значение |
|------|----------|
| **Core verb** | Собери тактику из амплуа → стекни ★ в магазине → **смотри матч** |
| **Пространство** | Полное поле top-down; 5v5; кружки с «ручками»; половины пересекаются |
| **Win** | Больше голов после 7 сегментов |
| **Fail** | Меньше голов к финалу |
| **Ввод** | Состав → магазин (витрина 3, скамейка 2, merge 3→★) → автобой; ×2 |
| **Анти-вектор** | Абстрактные теги без футбольного матча; физика 11v11; FM-Excel |

### Карта и комбо (ядро)

| Ось | Что это | Пример |
|-----|---------|--------|
| **Позиция** | зона слота | GK / DEF / MID / WING / FWD |
| **Амплуа** | лицо карты (1) | BWM, Poacher, Winger, ShotStop… |
| **Тактика** | TFT-трейт на карте (1) | Gegenpress, Tiki-Taka, Park the Bus, Counter, Wing Play |

**Тактика включается набором карт на поле** (не кнопкой меню): ×2 слабый стиль, ×3 полный, ×4 слом.  
Стиль меняет **картинку матча** (прессинг-отборы / короткие пасы / блок / контра / бровки) и отдельные FX.
