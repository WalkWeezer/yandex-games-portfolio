...
| **Ввод** | Lineup (карта→любая клетка; соперник **скрыт** до свистка, 1 из 5 пресетов) → shop → fight → shop… ×7; ×2 |
...
| Доход между сегментами | **+4** (+ мелкий бонус от остатка, опц.) | темп 7 сегментов |
| Сегментов матча | **7** | win/fail по голам |
