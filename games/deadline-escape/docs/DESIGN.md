...
- Игрок ходит только по полу; пропы, стены и окна **непроходимы**.
- **Периметр:** как мебель — seed от этажа; ~**80% проходов** (пол на краю), ~20% стены/окна короткими сегментами. Окна чаще на севере. С каждой стороны ≥1 проход.
- Мобы (включая директора) входят **только из проходов** из тумана за краем; директор (`ghost`) может лезть сквозь препятствия (~×0.5 скорости).
