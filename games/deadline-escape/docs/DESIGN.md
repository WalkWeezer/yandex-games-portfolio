...
- Клетки: `0` пол · `1` стол 1×1 · `3` растение · `4` кулер · `5`/`6` стол 2×1 (запад/восток). Код `2` (стена) **не используется**.
- Игрок ходит только по полу; пропы 1–2 клетки **непроходимы**. Внешних стен нет — край открыт в туман войны.
- Мобы спавнятся **из тумана за краем** сетки и заходят на крайний пол; директор (`ghost`) может лезть сквозь препятствия (~×0.5 скорости).
- Layout детерминирован от этажа; граф проходов связный; спавн отвергается без пути побега (`hasEscape`).
...
| Прочее | коллега / тайлы / пикапы — текущее |
...
| `near_miss` | босс на соседней клетке |
| `ui_click` | UI |
